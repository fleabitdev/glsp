use super::engine::{glsp, with_heap, Guard, Span};
...
pub trait DequeIndex: deque_index_private::Sealed {
    ...
    #[doc(hidden)]
    fn as_usize<A>(&self, arr: &A) -> GResult<usize>
    where
        A: DequeOps;
...
macro_rules! impl_arr_index_unsigned {
    ($type:ty) => {
        impl DequeIndex for $type {
            ...
            #[inline(always)]
            fn as_usize_excluded<A>(&self, arr: &A) -> GResult<usize>
            where
                A: DequeOps,
            {
                ensure!(
...
pub trait DequeOps: Sized + deque_ops_private::Sealed {
    ...
    */
    ...
    where
        F: FnMut(&Self::Item, &Self::Item) -> GResult<Ordering>;
...
fn generic_grow<T: Clone>(vec: &mut VecDeque<T>, start_to_add: usize, end_to_add: usize, fill: T) {
...
fn generic_shrink<T>(
    ...
    end_to_remove: usize,
        ...
        start_to_remove + end_to_remove <= vec.len(),
...
impl DequeOps for Arr {
    ...
    fn sort(&self) -> GResult<()> {
        self.sort_by(|v0, v1| match v0.partial_cmp(v1) {
...
impl DequeOps for Str {
    ...
    fn capacity(&self) -> usize {
        with_str_storage!(&*self.borrow(), vec, (), vec.capacity())
    }

    fn len(&self) -> usize {
        with_str_storage!(&*self.borrow(), vec, (), vec.len())
    }
    ...
    fn contains<C: IntoElement<char>>(&self, ch: C) -> GResult<bool> {
        ...
        self.borrow_mut_with_capacity_guard(|storage| {
            with_str_storage_mut!(storage, vec, (ch), Ok(vec.contains(&ch)))
        })
    }
...
impl DequeOps for Deque {
    ...
    fn swap<I1, I2>(&self, i: I1, j: I2) -> GResult<()>
    where
        I1: DequeIndex,
        I2: DequeIndex,
    {
...
impl<'a, T: DequeAccess<usize>, R: FromElement<T::Element>> DoubleEndedIterator
    for IterDequeTo<'a, T, R>
{
...
impl<'a, T: DequeAccess<usize>, R: FromElement<T::Element>> ExactSizeIterator
    for IterDequeTo<'a, T, R>
{
}
...
impl Allocate for Tab {
    ...
    fn owned_memory_usage(&self) -> usize {
        ...
        let buckets = if cap < 8 { cap + 1 } else { (cap * 8) / 7 };
...
impl Tab {
    ...
    pub(crate) fn with_capacity(capacity: usize) -> Tab {
        Tab {
            header: Header::new(),
            map: RefCell::new(FnvHashMap::with_capacity_and_hasher(
                capacity,
                Default::default(),
            )),
        }
    ...
    */
        ...
        V: IntoVal,
    {
        ...
        self.borrow_mut_with_capacity_guard(|map| match map.get_mut(&key) {
            Some(dst_value) => {
                self.write_barrier_slot(&key);
                self.write_barrier_slot(&value);
                *dst_value = value;
    ...
    pub fn is_deep_frozen(&self) -> bool {
        self.is_frozen()
            && self
                .entries()
                .iter()
                .all(|(key, value)| key.is_deep_frozen() && value.is_deep_frozen())
    }
...
impl<'a, 'b, K: FromVal> Iterator for IterTabKeysTo<'a, 'b, K> {
    ...
    fn next(&mut self) -> Option<GResult<K>> {
        self.iter
            .next()
            .map(|internal_key| K::from_slot(internal_key))
    }
...
impl<'a, 'b, V: FromVal> Iterator for IterTabValuesTo<'a, 'b, V> {
    ...
    fn next(&mut self) -> Option<GResult<V>> {
        self.iter
            .next()
            .map(|internal_value| V::from_slot(internal_value))
    }
