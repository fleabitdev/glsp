...
use super::engine::{glsp, stock_syms::*, Filename, Span, SpanStorage, Sym};
...
fn parse(parser: &mut Parser, text: &mut &str) -> GResult<Option<Val>> {
    ...
    let mut parsed_val: Option<Val> = match tok.tok_type {
        ...
        TokType::ArrClose => match parser.stack.pop() {
            Some(Form::Arr(arr)) => {
                arr.freeze();
                Some(Val::Arr(arr))
            }
        ...
        TokType::AccessClose => match parser.stack.pop() {
        ...
        TokType::StrOpen => {
            parser
                .stack
                .push(Form::Str(glsp::str(), StrEscape::NoEscape));
            None
        }
        TokType::StrClose => match parser.stack.pop() {
            Some(Form::Str(st, _)) => {
                st.freeze();
                Some(Val::Str(st))
            }
        ...
        TokType::StrResume => match parser.stack.pop() {
            Some(Form::PausedStr(arr)) => {
                parser
                    .stack
                    .push(Form::ResumedStr(arr, glsp::str(), StrEscape::NoEscape));
                None
            }
            _ => bail_at!(span(), "unexpected }}"),
        },
        TokType::StrChars => match parser.stack.last_mut() {
        ...
        TokType::RawStrClose => match parser.stack.pop() {
            Some(Form::RawStr(st)) => {
                st.freeze();
                Some(Val::Str(st))
            }
    ...
    while text.len() > 0 {
        ...
        if text.starts_with('\\') {
            ...
            let escaped_ch = match escape_chars.next().unwrap() {
                ...
                '0' => '\0',
                '"' => '"',
                'x' => match (escape_chars.next(), escape_chars.next()) {
