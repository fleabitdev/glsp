...
use super::engine::{glsp, with_heap, Span, Sym};
...
impl Coro {
    pub(crate) fn new(gfn: Raw<GFn>, regs: Vec<Slot>, stays: Vec<Option<Raw<Stay>>>) -> Coro {
        assert!(gfn.yields());
...
impl Allocate for Bytecode {
    ...
    fn visit_raws<V: Visitor>(&self, visitor: &mut V) {
        ...
        debug_assert!(self.start_regs[local_end_i..literal_start_i]
            .iter()
            .all(|slot| { *slot == Slot::Nil }));
    ...
    pub(crate) fn from_param_list(
        ...
        let (min_args, max_args) = match arg_limits {
            Some(pair) => {
                ensure_at!(
                    span,
                    basic_param_count == 0 && opt_param_count == 0 && rest_param.is_some(),
                    "the &arg-limits flag is only valid with \
    ...
    */
    ...
    pub(crate) fn wrangle_args(
        ...
        if arg_count < self.min_args {
            return Err(format!(
                "received {} argument{}, but expected at least {}",
                arg_count,
        ...
        if let Some(max_args) = self.max_args {
            if arg_count > max_args {
                return Err(format!(
                    "received {} argument{}, but expected no more than {}",
                    arg_count,
                    if arg_count == 1 { "" } else { "s" },
                    max_args
                ));
        ...
        if let Some(to_collect) = self.rest_param {
            ...
            if to_collect {
                let arr = with_heap(|heap| {
                    heap.recycler
                        .arr_from_iter(regs.drain(rest_base_index..))
                        .unwrap()
                });
...
impl From<SymBytes> for Sym {
    ...
    fn from(src: SymBytes) -> Sym {
        let bytes = src.0;
        Sym(
            u32::from_le_bytes([bytes[0], bytes[1], bytes[2], 0x00]),
            PhantomData,
        )
    }
}
...
impl TryFrom<isize> for JumpBytes {
    ...
    fn try_from(src: isize) -> GResult<JumpBytes> {
        if src < -8_388_608 || src > 8_388_607 {
            ...
            debug_assert!({
                let sign_extend = if bytes[2] & 0x80 == 0x80 {
                    0xffu8
                } else {
                    0x00
                };
