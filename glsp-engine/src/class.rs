...
use super::engine::{glsp, stock_syms::*, with_heap, with_vm, Guard, Sym, ToSym};
use super::error::GResult;
use super::gc::{Allocate, Header, Raw, Root, Slot, Visitor};
...
impl Binding {
    fn to_field_stack_entry(&self) -> FieldStackEntry {
        match *self {
            Binding::SimpleField(state_i, field_i) => FieldStackEntry::Field(state_i, field_i),
...
impl Class {
    ...
    fn lookup(&self, key: Sym) -> Option<Slot> {
        ...
        if let Some(binding) = self.bindings.get(&key) {
            match *binding {
                ...
                Binding::StackableField(mut stack_index) => loop {
    ...
    */
    pub fn mixins(&self) -> Root<Arr> {
        ...
        glsp::arr_from_iter(self.is.iter().map(|mixin| Slot::Class(mixin.clone()))).unwrap()
    }
}
...
impl CallableOps for Root<Class> {
    ...
    fn arg_limits(&self) -> (usize, Option<usize>) {
        let (gfn, has_nbi) = match self.states.get(&MAIN_SYM).unwrap().init {
            ...
            Some(MetBinding::Stackable(_, i)) => match self.met_stack[i as usize] {
...
impl Obj {
        ...
        A: Clone + IntoCallArgs,
    {
            ...
            !class.is_mixin,
            "{} is a mixin; mixins cannot be instantiated",
            class.name.unwrap()
        );
    ...
    */
    ...
    fn lookup(&self, key: Sym) -> Lookup {
        ...
        let state_is_accessible = |state_index: u8| states_enabled & (1 << state_index as u32) != 0;
        ...
        if let Some(binding) = self.class.bindings.get(&key) {
            match *binding {
                ...
                Binding::Met(ref met_binding) => match self.lookup_met(storage, met_binding) {
    ...
    */
    pub fn has_met<S: ToSym>(&self, key: S) -> GResult<bool> {
        match self.lookup(key.to_sym()?) {
            ...
            Lookup::PropGetter(getter) => match self.invoke_method(&getter, &())? {
    ...
    pub fn get_if_present<S: ToSym, V: FromVal>(&self, key: S) -> GResult<Option<V>> {
        if self.storage.borrow().is_some() {
            match self.lookup(key.to_sym()?) {
                ...
                Lookup::PropGetter(getter) => Ok(Some(self.invoke_method(&getter, &())?)),
    ...
    */
    ...
    pub(crate) fn get_method(&self, method_name: Sym) -> GResult<Option<(Slot, bool, bool, Slot)>> {
        Ok(match self.lookup(method_name) {
            Lookup::FieldOrConst(slot) => match slot {
    ...
    */
        ...
        R: FromVal,
    {
        ...
        if self.storage.borrow().is_some() {
            let slot = match self.lookup(sym) {
                Lookup::Met(met_lookup) => return Ok(Some(self.invoke_method(&met_lookup, args)?)),
        ...
        R: FromVal,
    {
        with_vm(|vm| {
            ...
            stacks.regs.push(Slot::Obj(
                self.storage.borrow().as_ref().unwrap().raw_self.clone(),
            ));
    ...
    fn set_impl<S: ToSym, V: IntoVal>(&self, key: S, value: V) -> GResult<Option<&'static str>> {
            ...
            self.storage.borrow().is_some(),
            "attempted to mutate a field on a killed obj"
        );
    ...
    */
        ...
        A: Clone + IntoCallArgs,
    {
        let sym = state_name.to_sym()?;
            ...
            self.class.states.contains_key(&sym),
            "attempted to enable the nonexistent state '{}'",
            sym
        );
        ensure!(
            !self.header.frozen(),
            "attempted to enable the state '{}' on a frozen obj",
            sym
        );
        ensure!(
            self.storage.borrow().is_some(),
            "attempted to enable the state '{}' on a killed obj",
            sym
        );
    ...
    fn enable_state<A: IntoCallArgs>(&self, state_name: Sym, args: A) -> GResult<()> {
        let mut storage_ref = self.storage.borrow_mut();
        ...
        let state_bit = 1 << state_ref.index as u32;
            ...
            storage.states_enabled & state_bit == 0,
            "attempted to enable the state {}, which is already enabled",
            state_name
        );
        ...
        //check that we're not violating `requires` or `excludes` by enabling this state
            ...
            (states_enabled & state_ref.requires) == state_ref.requires,
            "attempted to enable {}, but it wraps a method in a state which is not enabled",
            state_ref.name
        );
        ensure!(
            (states_enabled & state_ref.excludes) == 0,
            "attempted to enable {}, but \
                this caused a method name collision with another state",
            state_ref.name
        );
        ...
        if let Some(ref init_binding) = state_ref.init {
            ...
            //if there's no initializer, check that we received zero args
                ...
                args.arg_count() == 0,
                "too many arguments to state {}'s initializer: \
                    expected 0, received {}",
                state_name,
                args.arg_count()
            );
    ...
    */
    pub fn disab<S: ToSym>(&self, state_name: S) -> GResult<()> {
        let state_name_sym = state_name.to_sym()?;
            ...
            state_name_sym != MAIN_SYM,
            "the `Main` state cannot be disabled"
        );
    ...
    fn disab_impl<S: ToSym>(&self, state_name: S) -> GResult<()> {
        let state_name_sym = state_name.to_sym()?;
            ...
            !self.header.frozen(),
            "attempted to disable the state '{}' on a frozen obj",
            state_name_sym
        );
        ensure!(
            self.storage.borrow().is_some(),
            "attempted to disable the state '{}' on a killed obj",
            state_name_sym
        );
        ...
        let state_ref = match self.class.states.get(&state_name_sym) {
            ...
            None => bail!(
                "attempted to disable the nonexistent state '{}'",
                state_name_sym
            ),
        };
        ...
        for fini_binding in state_ref.finis.iter().rev() {
            ...
            let fini_lookup = self
                .lookup_met(&storage.as_ref().unwrap(), &fini_binding)
                .unwrap();
            drop(storage);
    ...
    fn disable_state(&self, state_name: Sym) -> GResult<()> {
        ...
        let state_bit = 1 << state_ref.index as u32;
            ...
            storage.states_enabled & state_bit != 0,
            "attempted to disable the state {}, which is already disabled",
            state_name
        );

        //check that we're not violating `required_by` when we disable this state
        ensure!(
            (storage.states_enabled & state_ref.required_by) == 0,
            "attempted to disable {}, \
                but an active state is currently wrapping one of its methods",
            state_ref.name
        );
...
impl RawClass {
    ...
    fn from_tab(tab: &Tab) -> GResult<RawClass> {
        ...
        for maybe_state_tab in states_arr.iter_to::<Raw<Tab>>() {
            let state_tab = maybe_state_tab?;
                ...
                states.len() <= 32,
                "{} states in a class, but the limit is 31",
                states.len() - 1
            );
            ...
            let bindee = match tag {
                ...
                PROP_SYM | WRAP_PROP_SYM | WILDCARD_WRAP_PROP_SYM => {
                    ...
                    match tag {
                        ...
                        WRAP_PROP_SYM => RawBindee::WrapProp(
                            qualified_get,
                            qualified_set,
                            arr.get(6)?,
                            getter,
                        ...
                        WILDCARD_WRAP_PROP_SYM => RawBindee::WildcardWrapProp(
                            qualified_get,
                            qualified_set,
                            getter,
                            setter,
        ...
        if !self.is_mixin && self.mixins.len() > 0 {
            ...
            for mixin in &self.mixins {
                ...
                let mixin_name = mixin.name.unwrap();
                    ...
                    names.insert(mixin_name),
                    "duplicate state or mixin name {}",
                    mixin_name
                );
    ...
    fn apply_mixin(&mut self, mixin: Raw<Class>) -> GResult<()> {
        ...
        let old_mains = self
            .bindings
            .iter()
            .filter(|rb| rb.state_name == MAIN_SYM)
            .count();
        let new_mains = raw_mixin
            .bindings
            .iter()
            .filter(|rb| rb.state_name == MAIN_SYM)
            .count();
        assert!(self.bindings[old_mains..]
            .iter()
            .all(|rb| rb.state_name != MAIN_SYM));
        assert!(raw_mixin.bindings[new_mains..]
            .iter()
            .all(|rb| rb.state_name != MAIN_SYM));

        self.bindings
            .splice(0..0, raw_mixin.bindings[..new_mains].iter().cloned());
        self.bindings.splice(
            new_mains + old_mains..new_mains + old_mains,
        ...
        self.states
            .splice(1..1, raw_mixin.states[1..].iter().cloned());

        ensure!(
            self.states.len() <= 32,
            "a single class may not contain more than 31 states"
        );
...
impl ClassBuilder {
    fn new(raw_class: RawClass) -> GResult<ClassBuilder> {
        ...
        for state in &raw_class.states {
                ...
                states.insert(state.name, state.clone()).is_none(),
                "duplicate state {}",
                state.name
            );
        }
    ...
    fn build(mut self) -> GResult<Class> {
        ...
        for init in &self.raw_class.inits {
            if init.state_name == MAIN_SYM {
                ...
                let state = self.states.get_mut(&init.state_name).unwrap();
                    ...
                    state.init.is_none(),
                    "multiple (init-state) forms in {}",
                    state.name
                );
                state.init = Some(MetBinding::Simple(state.index, init.gfn.clone(), false));
            }
        }
        ...
        if main_inits.len() == 1 {
            ...
            let state = self.states.get_mut(&MAIN_SYM).unwrap();
            state.init = Some(MetBinding::Stackable(
                state.index,
                self.met_stack.len() as u16,
            ));
        ...
        for fini in &self.raw_class.finis {
            if fini.state_name == MAIN_SYM {
                ...
                let state = self.states.get_mut(&fini.state_name).unwrap();
                    ...
                    state.finis.is_empty(),
                    "multiple (fini-state) forms in {}",
                    state.name
                );
                state
                    .finis
                    .push(MetBinding::Simple(state.index, fini.gfn.clone(), false));
            }
        }
                ...
                Category::FieldLike => {
                    ...
                    for binding in &group {
                        if (seen_states & (1 << binding.state_i as u32)) != 0 {
                            bail!(
                                "duplicate field/const {} in the {} state",
                                binding.qualified,
                                binding.state_name
                            );
                        }
                    ...
                    if group.len() > 1 {
                        ...
                        for raw_binding in group.iter().rev() {
                            let binding = self.bind_field_like(raw_binding)?;
                                ...
                                .bindings
                                .insert(raw_binding.qualified, binding.clone())
                                .is_none());
                        ...
                        self.field_stack.push(FieldStackEntry::End);
                            ...
                            self.field_stack.len() <= u16::MAX as usize,
                            "too many shadowing fields/consts"
                        );

                        ensure!(self
                            .bindings
                            .insert(unqualified, Binding::StackableField(field_stack_start))
                            .is_none());
                    } else {
                ...
                Category::MetLike => {
                    ...
                    if stacked.len() > 1 {
                        ...
                        for raw_binding in stacked.iter().rev() {
                                ...
                                self.met_stack.len() <= u16::MAX as usize,
                ...
                Category::PropLike => {
                    ...
                    if stacked.len() > 1 {
                        ...
                        for i in 1..bindings.len() {
                            match bindings[i] {
                                Binding::Prop(ref get, ref set) => {
                                        ...
                                        has_getter == get.is_some() && has_setter == set.is_some(),
                        ...
                        for (i, raw_binding) in stacked.iter().rev().enumerate() {
                            if !matches!(raw_binding.bindee, RawBindee::WildcardWrapProp(..)) {
                                ...
                                let setter_met_binding = match setter_binding {
                                    ...
                                    .insert(
                                        raw_binding.qualified,
                                        Binding::Prop(getter_met_binding, setter_met_binding)
                                    )
                                    .is_none());
                            }
                        }
                        ...
                        //emit the unqualified binding
                            ...
                            .bindings
                            .insert(unqualified, Binding::Prop(getter_binding, setter_binding))
                            .is_none());
                    } else {
    ...
    fn bind_field_like(&mut self, raw_binding: &RawBinding) -> GResult<Binding> {
        ...
        match raw_binding.bindee {
            ...
            RawBindee::Const(ref slot) => Ok(Binding::SimpleConst(state_i, slot.clone())),
            _ => unreachable!(),
        }
    }
    ...
    fn bind_prop_like(&mut self, raw_binding: &RawBinding) -> Binding {
        ...
        let getter_binding = getter_gfn
            .as_ref()
            .map(|getter_gfn| MetBinding::Simple(state_i, getter_gfn.clone(), rni));

        let setter_binding = setter_gfn
            .as_ref()
            .map(|setter_gfn| MetBinding::Simple(state_i, setter_gfn.clone(), rni));

        Binding::Prop(getter_binding, setter_binding)
    }

    fn stack_mets(&mut self, raw_bindings: &VecDeque<RawBinding>) -> GResult<Vec<RawBinding>> {
        /*
        ...
        */
        ...
        for i in 0..dst.len() {
            for j in i + 1..dst.len() {
                ...
                self.states.get_mut(&dst[i].state_name).unwrap().excludes |= 1 << (state_j as u32);
                self.states.get_mut(&dst[j].state_name).unwrap().excludes |= 1 << (state_i as u32);
            }
        }
        ...
        while dst_i < dst.len() {
                    ...
                    | RawBindee::WrapProp(_, _, target_qualified, _, _) => {
                        if target_qualified == dst_qualified {
                            ...
                            if src_state_i != dst_state_i {
                                self.states.get_mut(&src_state).unwrap().requires |=
                                    1 << (dst_state_i as u32);
                                self.states.get_mut(&dst_state).unwrap().required_by |=
                                    1 << (src_state_i as u32);
                            }
            ...
            for i in starting_len..dst.len() {
                for j in i + 1..dst.len() {
                    ...
                    self.states.get_mut(&dst[i].state_name).unwrap().excludes |=
                        1 << (state_j as u32);
                    self.states.get_mut(&dst[j].state_name).unwrap().excludes |=
                        1 << (state_i as u32);
                }
            }
