...
use super::engine::{glsp, with_heap, RData, RFn};
    ...
    */
    pub fn len(&self) -> GIterLen {
        ...
        match *self.state.borrow() {
            ...
            StepBy(step_by, ref base) => match base.len() {
            ...
            Flatten(_, _) => Unknown,
            Cycle(ref base, ref collection, _) => match base {
                None => {
                    if collection.len() == 0 {
                        Exact(0)
                    } else {
    ...
    */
    ...
    pub(crate) fn raw_next(&self) -> Option<GResult<Slot>> {
        ...
        let result = match *state_ref {
            ...
            Repeat1(ref slot) => Some(Ok(slot.clone())),
            ...
            Enumerate(ref base, ref mut n) => match base.raw_next() {
                Some(Ok(slot)) => {
                    let return_n = *n;
                    *n += 1;
            ...
            Cloned(ref base) => match base.raw_next() {
                Some(Ok(slot)) => match slot.root().shallow_clone() {
                    Ok(cloned) => Some(Ok(Slot::from_val(&cloned))),
                    Err(err) => Some(Err(err)),
                },
                err_or_none => err_or_none,
            },
            DeepCloned(ref base) => match base.raw_next() {
                Some(Ok(slot)) => match slot.root().deep_clone() {
                    Ok(cloned) => Some(Ok(Slot::from_val(&cloned))),
                    Err(err) => Some(Err(err)),
                },
                err_or_none => err_or_none,
            },
            StepBy(step_by, ref base) => match base.raw_next() {
                Some(Ok(slot)) => {
            ...
            Map(ref raw_callable, ref base) => match base.raw_next() {
                Some(Ok(slot)) => {
            ...
            Filter(ref raw_callable, ref base) => loop {
                match base.raw_next() {
                    Some(Ok(slot)) => {
            ...
            Flatten(ref base, ref mut cur_giter) => loop {
                if (*cur_giter).is_none() {
                    match base.raw_next() {
                        Some(Ok(slot)) => {
                            let val = slot.root();
                            if !val.is_iterable() {
                                break Some(Err(error!("flatten received {}", val.a_type_name())));
                            }
            ...
            TakeWhile(ref raw_callable, ref base) => match base.raw_next() {
                Some(Ok(item)) => {
            ...
            Skip(ref mut remaining, ref base) => loop {
                if *remaining == 0 {
                    break base.raw_next();
                }
            ...
            SkipWhile(ref raw_callable, ref base) => {
                if let Some(raw_callable) = raw_callable {
                    loop {
                        match base.raw_next() {
                            Some(Ok(item)) => {
                                let result: GResult<Val> =
                                    glsp::call(&raw_callable.root(), (&item,));
    ...
    pub(crate) fn raw_next_back(&self) -> Option<GResult<Slot>> {
        ...
        let result = match *state_ref {
            ...
            FRnExclusive(..) => Some(Err(error!("floating-point ranges are not double-ended"))),
            FRnInclusive(..) => Some(Err(error!("floating-point ranges are not double-ended"))),
            RnOpen(..) => Some(Err(error!("half-open ranges are not double-ended"))),
            FRnOpen(..) => Some(Err(error!("half-open ranges are not double-ended"))),
            ...
            CoroResults(_) => Some(Err(error!("coro iterators are not double-ended"))),
            Once1(ref slot) => {
                let result = slot.clone();
                *state_ref = Empty;
                Some(Ok(result))
            ...
            RepeatWith(_) => Some(Err(error!("repeat-with iterators are not double-ended"))),
            AccessArr(ref arr, ref giter) => {
            ...
            Windows(window_len, ref arr) => {
                if arr.len() < window_len as usize {
                    ...
                    for i in 1..window_len + 1 {
                        window
                            .push_start(arr.get::<Slot>(-(i as i32)).unwrap())
                            .unwrap();
                    }
                    arr.pop::<Slot>().unwrap();
                    Some(Ok(Slot::Arr(window.to_raw())))
            ...
            Enumerate(_, _) => Some(Err(error!("enumerate iterators are not double-ended"))),
            Cloned(ref base) => match base.raw_next_back() {
                Some(Ok(slot)) => match slot.root().shallow_clone() {
                    Ok(cloned) => Some(Ok(Slot::from_val(&cloned))),
                    Err(err) => Some(Err(err)),
                },
            ...
            DeepCloned(ref base) => match base.raw_next_back() {
                Some(Ok(slot)) => match slot.root().deep_clone() {
                    Ok(cloned) => Some(Ok(Slot::from_val(&cloned))),
                    Err(err) => Some(Err(err)),
                },
            ...
            StepBy(_, _) => Some(Err(error!("step-by iterators are not double-ended"))),
            Map(ref raw_callable, ref base) => match base.raw_next_back() {
                Some(Ok(slot)) => {
                    let result = glsp::call(&raw_callable.root(), &[slot]);
                    Some(result.map(|val| Slot::from_val(&val)))
                }
            ...
            Zip(_) => Some(Err(error!("zip iterators are not double-ended"))),
            Chain(ref arr) => loop {
                if arr.len() == 0 {
                    break None;
                }
            ...
            Flatten(_, _) => Some(Err(error!("flatten iterators are not double-ended"))),
            Cycle(_, _, _) => Some(Err(error!("cycle iterators are not double-ended"))),
            Take(_, _) => Some(Err(error!("take iterators are not double-ended"))),
...
impl GIterState {
    fn shallow_clone(&self) -> GIterState {
        ...
        match self {
            ...
            Flatten(base, cur_iter) => Flatten(
                base.shallow_clone().to_raw(),
                cur_iter
                    .as_ref()
                    .map(|cur_iter| cur_iter.shallow_clone().to_raw()),
            ),
            Cycle(base, arr, n) => Cycle(
