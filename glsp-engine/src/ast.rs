...
use super::engine::{stock_syms::*, Span, Sym, SymKind};
...
impl<T> Range<T> {
    ...
    pub(crate) fn id_at(&self, i: usize) -> Id<T> {
        ...
        Id(
            NonZeroU32::new(self.0.get() + i as u32).unwrap(),
            PhantomData,
        )
    }
}
...
fn val_to_node(ast: &mut Ast, val: &Val, span: Span) -> GResult<Node> {
    ...
    match *val {
        ...
        Val::Sym(sym) => Ok(Node(span, Expr::Var(sym))),
        ...
        Val::Arr(ref arr) => arr_to_node(ast, arr),
        Val::Str(ref root) => {
...
fn arr_to_node(ast: &mut Ast, arr: &Root<Arr>) -> GResult<Node> {
    ...
    match root {
        ...
        //(.name ...) -> (call-met 'name ...)
        Val::Arr(root)
            if root.len() >= 1 && root.get::<Val>(0).unwrap() == Val::Sym(MET_NAME_SYM) =>
        {
            ...
            let met_name = match root.get::<Val>(1).unwrap() {
                ...
                val => bail_at!(
                    span,
                    "invalid (met-name ...) form: argument is {}",
                    val.a_type_name()
                ),
            };
        ...
        Val::Arr(root)
            if root.len() >= 1 && root.get::<Val>(0).unwrap() == Val::Sym(QUESTION_MARK_SYM) =>
        {
            ensure_at!(span, root.len() == 2, "invalid (? ...) callee");

            let met_name_arr = match root.get::<Val>(1).unwrap() {
                Val::Arr(arr)
                    if arr.len() >= 1 && arr.get::<Val>(0).unwrap() == Val::Sym(MET_NAME_SYM) =>
                {
                    arr
                }
                Val::Sym(name) => bail_at!(
                    span,
                    "(? {}) is not a valid callee. \
                                         did you mean (? .{})?",
                    name,
                    name
                ),
...
fn special_to_node(ast: &mut Ast, name: Sym, args: &[Val], span: Span) -> GResult<Node> {
    ...
    if args.len() < min_args {
        bail_at!(
            span,
            "the {} special form expects at least {} argument{}, but only {} were \
                 provided",
            name,
            min_args,
    ...
    if let Some(max_args) = max_args {
        if args.len() > max_args {
            bail_at!(
                span,
                "the {} special form expects no more than {} argument{}, but {} were \
                     provided",
                name,
                max_args,
...
fn finish_block_to_node(ast: &mut Ast, args: &[Val], span: Span) -> GResult<Node> {
    let block_name = match args[0] {
        ...
        _ => bail_at!(
            span,
            "the first argument to (finish-block) must be a symbol literal"
        ),
    };
...
fn restart_block_to_node(_ast: &mut Ast, args: &[Val], span: Span) -> GResult<Node> {
    let block_name = match args[0] {
        ...
        _ => bail_at!(
            span,
            "the first argument to (restart-block) must be a symbol literal"
        ),
    };
        ...
        match args[i] {
            ...
            Val::Sym(FLAG_ARG_LIMITS_SYM) => {
                ensure_at!(
                    span,
                    args.len() >= i + 2,
                    "invalid &arg-limits flag in (fn)"
                );
                ensure_at!(
                    span,
                    arg_limits.is_none(),
                    "duplicate &arg-limits flag in (fn)"
                );
                ...
                let max_args = match args[i + 2] {
                    Val::Int(i) if i >= 0 => {
                        ensure_at!(
                            span,
                            i as usize >= min_args,
                            "minimum arg count {} is \
                                   greater than maximum arg count {}",
                            min_args,
                            i
                        );
                        Some(i as usize)
            ...
            match param {
                Val::Sym(name) => {
                    ensure_at!(
                        span,
                        name != UNDERSCORE_SYM,
                        "unexpected _ in (fn) special form"
                    );
                    ensure_at!(
                        span,
                ...
                Val::Arr(param_arr) if param_arr.len() > 0 => match param_arr.get(0)? {
                    Val::Sym(QUESTION_MARK_SYM) => {
                        ensure_at!(
                            span,
                            (param_arr.len() == 2 || param_arr.len() == 3)
                                && param_arr.get::<Val>(1)?.is_sym()
                                && param_arr.get::<Val>(1)? != Val::Sym(UNDERSCORE_SYM),
                            "invalid param {} in (fn) special form",
                            &param_arr
                        );
                    ...
                    Val::Sym(SPLAY_SYM) => {
                        ensure_at!(
                            span,
                            param_arr.len() == 2 && param_arr.get::<Val>(1)?.is_sym(),
                            "invalid param {} in (fn) special form",
                            &param_arr
                        );
                    ...
                    _ => {
                        bail_at!(
                            span,
                            "unexpected parameter {} in (fn) special form",
                            param_arr
                        )
                    }
                },
