...
pub fn init_engine(text: String, filename: String, rand_seed: f32) -> Result<(), JsValue> {
    ...
    ENGINE.with(|engine| {
        ...
        *option = None;
        ...
        let result: Result<(), JsValue> = runtime
            .run(|| {
                ...
                width = match glsp::global::<_, Val>("play:width") {
                    Ok(Val::Int(width)) if width >= MIN_SIZE && width <= MAX_SIZE => width as u32,
                    Ok(val) => {
                        let msg = format!(
                            "invalid play:width value {}\n(should be an integer \
                                       between {} and {})",
                            val, MIN_SIZE, MAX_SIZE
                        );
                        return Ok(Err(JsValue::from_str(&msg)));
                    }
                    Err(_) => return Ok(Err(JsValue::from_str("play:width is not defined"))),
                };
                ...
                height = match glsp::global::<_, Val>("play:height") {
                    ...
                    Ok(val) => {
                        let msg = format!(
                            "invalid play:height value {}\n(should be an integer \
                                       between {} and {})",
                            val, MIN_SIZE, MAX_SIZE
                        );
                        return Ok(Err(JsValue::from_str(&msg)));
                    }
                    Err(_) => return Ok(Err(JsValue::from_str("play:height is not defined"))),
                };
                ...
                title = match glsp::global::<_, Val>("play:title") {
                    ...
                    Err(_) => return Ok(Err(JsValue::from_str("play:title is not defined"))),
                };
                ...
                blurb = match glsp::global::<_, Val>("play:blurb") {
                    ...
                    Err(_) => return Ok(Err(JsValue::from_str("play:blurb is not defined"))),
                };
...
pub fn width() -> Result<u32, JsValue> {
    with_engine(|engine| Ok(engine.width))
}

#[wasm_bindgen]
pub fn height() -> Result<u32, JsValue> {
    with_engine(|engine| Ok(engine.height))
}

#[wasm_bindgen]
pub fn title() -> Result<String, JsValue> {
    with_engine(|engine| Ok(engine.title.clone()))
}

#[wasm_bindgen]
pub fn blurb() -> Result<String, JsValue> {
    with_engine(|engine| Ok(engine.blurb.clone()))
}
        ...
        engine
            ...
            .run(|| {
                ...
                let _: Val = match glsp::call(&update, (dt,)) {
                    Ok(val) => val,
                    Err(glsp_err) => return Ok(Err(JsValue::from_str(&glsp_err.to_string()))),
                };
