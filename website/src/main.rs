use image::{self, imageops, DynamicImage, GenericImageView, Rgba};
...
const DEMOS: &[&str] = &["minefinder.glsp", "quadris.glsp", "tennis.glsp"];
...
fn render_arg_brief(arg: &ApiArg) -> String {
    match &arg.reps {
        ...
        .into_string(),
        ArgReps::Optional(None) => (html! {
            "(" span.subtle { "? " (arg.name) } ")"
        })
        .into_string(),
        ArgReps::Optional(Some(default)) => (html! {
            "(" span.subtle { "? " (arg.name) " " (default) } ")"
        })
        .into_string(),
        ArgReps::ZeroOrMore | ArgReps::OneOrMore | ArgReps::TwoOrMore => (html! {
            span.subtle { ".." (arg.name) }
        })
        .into_string(),
    }
...
impl ApiType {
    ...
    fn a_name(&self) -> String {
        ...
        let (prefix, linked_text) = match *self {
            ...
            Coro => ("a ", "coro"),
            RData => ("an ", "rdata"),
            Or2(ref tuple) => return format!("{} or {}", tuple.0.a_name(), tuple.1.a_name()),
            Or3(ref tuple) => {
                return format!(
                    "{}, {}, or {}",
                    tuple.0.a_name(),
    ...
    fn names(&self) -> String {
        ...
        let text = match *self {
            ...
            Coro => "coros",
            RData => "rdatas",
            Or2(ref tuple) => return format!("{} or {}", tuple.0.names(), tuple.1.names()),
            Or3(ref tuple) => {
                return format!(
                    "{}, {}, or {}",
                    tuple.0.names(),
...
fn convert_index(categories: &[RawCategory]) -> Page {
    ...
    for category in categories {
        ...
        let rows = if category
            .apis
            .iter()
            .any(|api| api.starts_subcategory.is_some())
        {
...
fn convert_alphabetic_index(categories: &[RawCategory]) -> Page {
    ...
    for (i, bucket) in buckets.iter().enumerate() {
        let heading = if i == 0 {
            ...
            char::try_from('A' as u32 + (i - 1) as u32)
                .unwrap()
                .to_string()
        };
    ...
    Page {
        ...
        parent_href: None,
        link_list,
        content: Content::AlphabeticIndex { tables },
    ...
    for arg in &api.args {
        ...
        args.push(ApiArg {
            ...
            reps: match reps_str {
                ...
                Some(st) if st.starts_with("?") => ArgReps::Optional(Some(st[1..].to_string())),
            ...
            for (i, filename) in see_also.iter().enumerate() {
                ...
                if page_name.is_code {
                    write!(
                        &mut builder,
                        "[`{}`]({})",
                        page_name.user_facing, page_name.href
                    )
                    .unwrap();
                } else {
                    write!(
...
fn build_search_index(categories: &[RawCategory]) -> String {
    ...
    for category in categories {
        ...
        write!(
            &mut search_index,
            "\t[{:?}, {}, {:?}],\n",
            page_name.user_facing, false, page_name.href
        )
        .unwrap();
        ...
        for api in &category.apis {
            ...
            write!(
                &mut search_index,
                "\t[{:?}, {}, {:?}],\n",
                page_name.user_facing, true, page_name.href
            )
            .unwrap();
            ...
            if api.kinds.contains(&Kind::Abbrv) {
                write!(
                    &mut search_index,
                    "\t[{:?}, {}, {:?}],\n",
                    page_name.filename, true, page_name.href
                )
                .unwrap();
            }
        }
...
fn maybe_transform_images() -> bool {
    ...
    let input_paths: Vec<PathBuf> = WalkDir::new("input")
        .into_iter()
        .filter_map(convert)
        .collect();
    let output_paths: Vec<PathBuf> = WalkDir::new("output")
        .into_iter()
        .filter_map(convert)
        .collect();
...
fn transform_images() {
    ...
    fs::copy(
        "input/patreon-button-light.png",
        "output/patreon-button-light.png",
    )
    .unwrap();
    fs::copy(
        "input/patreon-button-dark.png",
        "output/patreon-button-dark.png",
    )
    .unwrap();
}

fn make_sprites(dst_light: &str, dst_dark: &str, src: &str, palette: &HashMap<Rgba<u8>, Rgba<u8>>) {
    let light = image::open(src).unwrap();

    let mut dark = DynamicImage::new_rgba8(light.width(), light.height());
    for ((_, _, src), dst) in light
        .pixels()
        .zip(dark.as_mut_rgba8().unwrap().pixels_mut())
    {
        if src[3] == 0 {
            *dst = Rgba([0u8, 0, 0, 0]);
        } else {
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "cleaned up the output directory in {:?}",
    ...
    fs::copy(
        format!("input/{}", STYLESHEET),
        format!("output/{}", STYLESHEET),
    )?;
    fs::copy(
        format!("input/{}", TCOF_SPLASH_IMAGE),
        format!("output/{}", TCOF_SPLASH_IMAGE),
    )?;
    for name in &[GLSP_PLAYGROUND_JS, GLSP_PLAYGROUND_BG_WASM] {
        fs::copy(
            format!("input/playground/{}", name),
            format!("output/playground/{}", name),
        )?;
    }
    fs::write("output/.nojekyll", "")?;
    fs::copy(
        "input/playground/spritesheet.png",
        "output/playground/spritesheet.png",
    )?;
    fs::copy(
        "input/playground/spritesheet.json",
    ...
    fs::write(
        &format!("output/playground/{}", PLAYGROUND_JS),
        playground_js,
    )?;
    ...
    println!(
        "generated and rendered pages in {:?}",
        pages_start.elapsed()
    );
