use super::pat::{
    pat_from_forms, AssignStrategy, Matcher, MismatchStrategy, Pat, PlaceStrategy, SetStrategy,
};
use super::Std;
use glsp::{
    arr, bail, bail_at, ensure, error, macro_no_op, stock_syms::*, str, Arr, Callable, DequeAccess,
    DequeOps, EnvMode, FromVal, GResult, RGlobal, Rest, Root, Span, Sym, Val,
};
...
pub fn init(sandboxed: bool) -> GResult<()> {
    ...
    static OPT_SETTERS: [(&str, &str, Option<bool>); 1] = [("atsign", "atsign-opt=", Some(false))];
    ...
    for (accessor, setter, memoize_args) in &SETTERS {
        ...
        std.setters
            .insert(glsp::sym(accessor)?, (glsp::sym(setter)?, memoize_args));
    }
    ...
    match val {
        Val::Arr(arr) => {
            if arr.len() >= 1 && arr.get::<Val>(0).unwrap() == Val::Sym(QUESTION_MARK_SYM) {
                    ...
                    arr.len() == 2,
                    "? expects 1 argument, but received {}",
                    arr.len() - 1
                );
...
fn unwrap_slice_args(rest: &[Val]) -> GResult<Option<(Val, Val)>> {
    ...
    if let Some((to, from)) = &to_from {
            ...
            !is_splay_form(to) && !is_splay_form(from),
            "slice arguments cannot be splayed"
        );
    }
...
fn set_access(coll: Val, rest: Rest<Val>) -> GResult<Val> {
    ...
    if !args.is_empty() {
        if let Some(opt_index) = unwrap_question_mark(args[0].clone())? {
                ...
                args.len() == 1,
                "too many arguments to (= [coll (? idx)] new-val) form"
            );
...
fn remove(coll: Val, rest: Rest<Val>) -> GResult<Val> {
    if let Some((from, to)) = unwrap_slice_args(&rest)? {
        ...
        rest.len() == 1,
        "argument mismatch: expected 2 arguments, \
            received {}",
        rest.len() + 1
    );
...
fn matchesp(input_form: Val, pat_forms: Rest<Val>) -> GResult<Val> {
    ...
    let (pat, forms_consumed) = pat_from_forms(&pat_forms, false, Span::default())?;
        ...
        forms_consumed == pat_forms.len(),
        "unexpected end of pattern in (matches?)"
    );
...
fn fn_common(args: &[Val], atsign_params: bool) -> GResult<Val> {
    ...
    let pats = match &params_pat.matcher {
        ...
        _ => bail_at!(
            params_arr.span(),
            "in (fn), {} is not a valid array pattern",
            params_arr
        ),
    };
    ...
    for (pat_i, pat) in pats.iter().enumerate() {
        ...
        let opt_with_nontrivial_initializer = match &pat.matcher {
            Matcher::Opt(_, Some(val)) => match val {
                Val::Nil
                | Val::Bool(_)
                | Val::Int(_)
                | Val::Flo(_)
                ...
                Val::Arr(arr) if arr.len() == 2 && arr.get::<Val>(0)? == Val::Sym(QUOTE_SYM) => {
                    false
                }
                _ => true,
            },
        ...
        let nontrivial_opt_or_rest = match &pat.matcher {
            Matcher::Opt(sub_matcher, _) | Matcher::Rest(sub_matcher) => {
                pat.at.is_some() || pat.pred.is_some() || !matcher_is_trivial(sub_matcher)
            }
            _ => false,
        };

        if nontrivial_opt_or_rest || nonterminal_rest || opt_with_nontrivial_initializer {
    ...
    match strategy {
        FnStrategy::Trivial => {
            ...
            fn trivial_matcher_to_form(matcher: &Matcher) -> Val {
                match matcher {
                    ...
                    Matcher::Opt(sub_matcher, None) => Val::Arr(arr![
                        QUESTION_MARK_SYM,
                        trivial_matcher_to_form(sub_matcher)
                    ]),
                    Matcher::Opt(sub_matcher, Some(init_val)) => Val::Arr(arr![
                        QUESTION_MARK_SYM,
                        trivial_matcher_to_form(sub_matcher),
                        init_val.clone()
                    ]),
                    Matcher::Rest(sub_matcher) => {
                        Val::Arr(arr![SPLAY_SYM, trivial_matcher_to_form(sub_matcher)])
                    }
                    _ => panic!(),
                }
            }
            ...
            let trivial_params =
                glsp::arr_from_iter(pats.iter().map(|pat| trivial_matcher_to_form(&pat.matcher)))?;
        ...
        FnStrategy::Simple => {
            ...
            Ok(backquote!(
                "(fn ~..flags ~simple_params ~..pats_body ~..body)"
            ))
        }
        FnStrategy::Complex => {
            ...
            let (min_args, max_args) = params_pat.matcher.arg_limits();
                ...
                !flags.contains(&Val::Sym(FLAG_ARG_LIMITS_SYM)),
                "unexpected &arg-limits flag"
            );
...
fn arrow(pos: ArrowPos, first: Val, rest: &[Val]) -> GResult<Val> {
    ...
    for (i, val) in rest.iter().enumerate() {
        let (val, splayed) = match val {
            Val::Arr(arr)
                if arr.len() == 2 && arr.get::<Val>(0).unwrap() == Val::Sym(SPLAY_SYM) =>
            {
...
fn for_(args: Rest<Val>) -> GResult<Val> {
    let (_, forms_consumed) = pat_from_forms(&args, false, Span::default())?;
        ...
        args.len() >= forms_consumed + 2 && args[forms_consumed] == Val::Sym(IN_SYM),
...
fn forn(clause: Root<Arr>, body: Rest<Val>) -> GResult<Val> {
        ...
        clause.len() >= 2 && clause.len() <= 4 && clause.get::<Val>(0)?.is_sym(),
        "invalid forn clause {}",
        &clause
    );
...
fn set(std: &Std, args: Rest<Val>) -> GResult<Val> {
    ...
    fn set_clause(std: &Std, place: Val, new_val_form: Val) -> GResult<Val> {
        ...
        let place = place.unwrap_arr();
            ...
            place.len() >= 1 && place.get::<Val>(0)?.is_sym(),
            "invalid place {:?} passed to =",
            place
        );
        ...
        if callee == QUESTION_MARK_SYM {
            ...
            let opt_place = opt_place.unwrap_arr();
                ...
                opt_place.len() >= 1 && opt_place.get::<Val>(0)?.is_sym(),
                "invalid place {:?} passed to =",
                place
            );
    ...
    match args.len() {
        ...
        2 => {
                ...
                !is_splay_form(&args[1]),
                "the second argument to = cannot be splayed"
            );
    ...
    match place {
        ...
        Val::Arr(ref place) if place.len() >= 1 && place.get::<Val>(0)?.is_sym() => {
            ...
            let std = Std::borrow();
                ...
                std.setters.contains_key(&accessor),
                "unrecognized place form {:?}",
                place
            );
...
fn mul_assign(place: Val, by: Rest<Val>) -> GResult<Val> {
        ...
        by.len() >= 1,
        "mul! expects a place and at least one additional argument"
    );
    in_place(place, |place| {
        Ok(backquote!("(= ~&place (* ~&place ~..by))"))
    })
...
fn div_euclid_assign(place: Val, by: Rest<Val>) -> GResult<Val> {
    in_place(place, |place| {
        Ok(backquote!("(= ~&place (div-euclid ~&place ~..by))"))
    })
}
...
fn rem_euclid_assign(place: Val, by: Val) -> GResult<Val> {
    in_place(place, |place| {
        Ok(backquote!("(= ~&place (rem-euclid ~&place ~&by))"))
    })
}
...
fn seek_assign(place: Val, target: Val, step_by: Option<Val>) -> GResult<Val> {
    match step_by {
        Some(step_by) => in_place(place, |place| {
            Ok(backquote!(
                "
...
fn clamp_assign(place: Val, min: Val, max: Val) -> GResult<Val> {
    in_place(place, |place| {
        Ok(backquote!("(= ~&place (clamp ~&place ~&min ~&max))"))
    })
}
