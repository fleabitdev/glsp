use super::pat::{
    pat_from_forms, AssignStrategy, MismatchStrategy, Pat, PlaceStrategy, SetStrategy,
};
use super::Std;
use glsp::{
    arr, bail, bail_at, ensure, ensure_at, stock_syms::*, Arr, Class, DequeAccess, DequeOps,
    EnvMode, Expander, Expansion, FromVal, GFn, GResult, Obj, Rest, Root, Span, Sym, Tab, Val,
};
...
fn class_impl(std: &Std, clauses: &[Val], is_mixin: bool) -> GResult<Val> {
            ...
            if arr.len() > 0 && arr.get::<Val>(0)?.is_sym() {
                ...
                match tag {
                    NAME_SYM => {
                        ...
                        ensure_at!(
                            arr.span(),
                            arr.len() == 2 && arr.get::<Val>(1)?.is_sym(),
                            "invalid clause {}",
                            arr
                        );
    ...
    fn auto_gensym(arg: Val, auto_gensyms: &mut HashMap<Sym, Sym>) -> GResult<Val> {
        match arg {
            Val::Arr(arr) if arr.len() == 0 => Ok(Val::Arr(arr![])),

            //unlikely that we'd encounter a nested backquote, but if so it's a no-op
            Val::Arr(arr) if arr.get::<Val>(0)? == Val::Sym(BACKQUOTE_SYM) => Ok(Val::Arr(arr)),
...
fn process_class_state(
    ...
    for clause in clauses.iter().cloned() {
        ...
        ensure_at!(
            clause.span(),
            clause.len() > 0 && clause.get::<Val>(0)?.is_sym(),
            "invalid clause {}",
            clause
        );
        ...
        match tag {
            ...
            FIELD_SYM => {
                ...
                while !rest.is_empty() {
                    ...
                    for field_name in &names {
                        ...
                        ensure_at!(
                            clause.span(),
                            !binding_names.contains_key(field_name),
            ...
            CONST_SYM => {
                ...
                while !rest.is_empty() {
                    ...
                    for (i, const_name) in names.iter().enumerate() {
                        ...
                        ensure_at!(
                            clause.span(),
                            !binding_names.contains_key(const_name),
                            "the name {} is already bound",
                            qualified_name
                        );
            ...
            MET_SYM => {
                ensure_at!(
                    clause.span(),
                    clause.len() >= 3
                        && clause.get::<Val>(1)?.is_sym()
                        && clause.get::<Val>(2)?.is_arr(),
                    "invalid met clause"
                );
                ...
                ensure_at!(
                    clause.span(),
                    !binding_names.contains_key(&met_name),
                    "the name {} is already bound",
                    qualified_name
                );
            ...
            WRAP_SYM => {
                ...
                if target_state == UNDERSCORE_SYM {
                    ...
                    ensure_at!(
                        clause.span(),
                        !binding_names.contains_key(&unqualified),
                        "the name {} is already bound",
                        qualified
                    );
            ...
            PROP_SYM => {
                ...
                let (prop_name, initializer_form, getter_form, setter_form) =
                ...
                ensure_at!(
                    clause.span(),
                    !binding_names.contains_key(&backing_name),
                    "the name {} is already bound",
                    qualified_backing_name
                );

                let (pat, _) = pat_from_forms(&[Val::Sym(backing_name)], false, clause.span())?;
                ...
                ensure_at!(
                    clause.span(),
                    !binding_names.contains_key(&prop_name),
                    "the name {} is already bound",
                    qualified_name
                );

                let getter_form = getter_form
                    .map(|getter_form| {
            ...
            INIT_SYM | INIT_STATE_SYM | INIT_MIXIN_SYM | FINI_SYM | FINI_STATE_SYM
            | FINI_MIXIN_SYM | FSM_SYM | STATE_SYM | STATEX_SYM => (),
    ...
    for clause in clauses.iter().cloned() {
        ...
        match tag {
            INIT_SYM | INIT_MIXIN_SYM | INIT_STATE_SYM => {
                ensure_at!(
                    clause.span(),
                    clause.len() >= 2 && clause.get::<Val>(1)?.is_arr(),
                    "invalid ({}) form",
                    tag
                );
            ...
            STATE_SYM | STATEX_SYM => {
                ensure_at!(
                    clause.span(),
                    clause.len() >= 2 && clause.get::<Val>(1)?.is_sym(),
            ...
            FSM_SYM => {
                ...
                for fsm_clause in &fsm_expanded {
                    ...
                    ensure_at!(
                        fsm_clause.span(),
                        fsm_tag == STATE_SYM || fsm_tag == STATEX_SYM,
                        "invalid fsm tag {}: the immediate children of an fsm form \
                                must be #n, (state) or (state*)",
                        fsm_tag
                    );
                    ...
                    ensure_at!(
                        fsm_clause.span(),
                        fsm_clause.get::<Val>(1)?.is_sym(),
                        "{} clause missing a name",
                        fsm_tag
                    );
    ...
    if let Some(init_clause) = init_clause {
        ...
        init_forms.push(backquote!(
            "(arr '~state_name ~init_method_form ~requires_next_index)"
        ));
    }
...
fn const_initializer_to_form(
    ...
    fn recursively_transform(tab_name: Sym, form: Val, const_tab_flag: &mut bool) -> GResult<Val> {
        match form {
            Val::Arr(arr) if arr.len() == 2 && arr.get::<Val>(0)? == Val::Sym(ATSIGN_SYM) => {
                ensure_at!(
                    arr.span(),
                    arr.get::<Val>(1)?.is_sym(),
                    "only @sym-literal forms \
            ...
            Val::Arr(arr) => Ok(Val::Arr(span_map(&arr, |form| {
    ...
    let transformed_initializer =
        recursively_transform(tab_name, expanded_initializer, &mut const_tab_flag)?;
...
fn method_clause_to_form(
    ...
    */
            ...
            for pat_name in &pat_names {
                ...
                if init_at_params.contains(&pat_name) {
                    ensure_at!(
                        clause.span(),
                        initializer.is_none(),
                        "the field {} cannot have \
                               both an initializer form and an @-param",
                        qualified_sym
                    );
    ...
    fn recursively_transform(
        ...
        let result = match form {
                        ...
                        || arr.get::<Val>(0)? == Val::Sym(ATSIGN_OPT_SYM)) =>
            {
                ...
                match arr.get::<Val>(1)? {
                    Val::Sym(SELF_SYM) => Ok(Val::Sym(self_name)),
                    Val::Sym(CLASS_SYM) => Ok(backquote!("(class-of ~self_name)")),
                    Val::Sym(CLASS_NAME_SYM) => {
                        Ok(backquote!("(class-name (class-of ~self_name))"))
                    }
                    Val::Sym(STATE_NAME_SYM) => Ok(backquote!("'~state_name")),
                    Val::Sym(FIELD_SYM) => {
                        ensure_at!(
                            arr.span(),
                            prop_backing_name.is_some(),
            ...
            //((? @form) ...), which the (?) macro will have expanded to ((atsign-opt form) ...)
            Val::Arr(ref arr)
                if arr.len() >= 1 && arr.get::<Val>(0)?.is_arr() && {
                ...
                } =>
            {
                ...
                match second {
                    ...
                    Val::Sym(FIELD_SYM) => {
                        ...
                        if is_opt {
                            Ok(backquote!(
                                "(call-met-opt '~prop_backing_name ~self_name
                                                          ~..transformed_arg_forms)"
                            ))
                        } else {
                            Ok(backquote!(
                                "(call-met '~prop_backing_name ~self_name
                                                      ~..transformed_arg_forms)"
                            ))
                        }
                    }
                    Val::Sym(callee_name) => {
                        if is_opt {
                            Ok(backquote!(
                                "(call-met-opt '~callee_name ~self_name 
                                                          ~..transformed_arg_forms)"
                            ))
                        } else {
                            Ok(backquote!(
                                "(call-met '~callee_name ~self_name 
                                                      ~..transformed_arg_forms)"
                            ))
                        }
                    }
            ...
            //any other arr
            Val::Arr(ref arr) => Ok(Val::Arr(span_map(&arr, |form| {
    ...
    //finished!
    Ok(backquote!(
        "(fn &name ~qualified_name ~transformed_params_arr ~..transformed_body)"
    ))
}
...
fn split_wrap_target(span: Span, target_name: Sym) -> GResult<(Sym, Sym)> {
    ...
    ensure_at!(
        span,
        name_str.bytes().filter(|ch| *ch == b':').count() == 1,
        "invalid wrap target {}",
        target_name
    );
...
fn process_prop_clause(
    ...
    ensure_at!(
        clause.span(),
        clause.len() >= 3 && clause.len() <= 5 && clause.get::<Val>(1)?.is_sym(),
        "invalid ({}) clause",
        tag
    );
    ...
    for i in 2..clause.len() {
        ...
        if i == 2 {
            ensure_at!(
                clause.span(),
                tag == PROP_SYM,
                "(wrap-prop) clauses may not include \
                       an initializer"
            );
...
fn bind_classmacro(std: &mut Std, name: Sym, gfn: Root<GFn>) -> GResult<()> {
        ...
        !std.classmacros.contains_key(&name),
        "duplicate classmacro {}",
        name
    );
...
fn defstruct(name: Sym, clauses: Rest<Val>) -> GResult<Val> {
    ...
    for clause in clause_stack.iter().rev() {
        match clause {
            ...
            Val::Sym(_) => bail!(
                "invalid struct clause: all field names must be grouped together \
                                at the beginning of the struct definition"
            ),
            val => bail!(
                "invalid struct clause: expected an arr, receved {}",
                val.a_type_name()
            ),
        }
    }
...
fn struct_constructor_macro(
    ...
    for (i, arg) in args.iter().enumerate() {
        let (field_name, init_form) = match *arg {
            Val::Sym(field_name) => (field_name, Val::Sym(field_name)),
            Val::Arr(ref arr) if arr.len() == 2 && arr.get::<Val>(0)?.is_sym() => {
                match arr.get::<Sym>(0)? {
                    ...
                    field_name => (field_name, arr.get(1)?),
                }
            }
    ...
    match remaining_names.len() {
        0 => (),
        1 => bail!(
            "{} struct constructor is missing the field {}",
            name,
            remaining_names.drain().next().unwrap().0
        ),
...
fn isp(arg: Val, class: Val) -> bool {
    match (arg, class) {
        ...
        (Val::RData(rdata), Val::Sym(sym)) => match rdata.rclass() {
...
fn class_of(arg: Val) -> GResult<Val> {
    match arg {
        ...
        Val::RData(rdata) => match rdata.rclass() {
            Some(rclass) => Ok(Val::Sym(rclass.name())),
            None => Ok(Val::Nil),
        },
        val => bail!(
            "expected an obj or an rdata, received {}",
            val.a_type_name()
        ),
    }
}
...
fn call_meth(method_name: Sym, rcv: Val, args: Rest<Val>) -> GResult<Val> {
    match rcv {
        ...
        val => bail!(
            "expected an obj, class or rdata, received {}",
            val.a_type_name()
        ),
    }
}
