use super::Std;
use glsp::{
    bail, ensure, stock_syms::*, Arr, Callable, Class, Deque, DequeAccess, DequeAccessRange,
    DequeOps, EprWriter, FromVal, GIterLen, GResult, IntoVal, Iterable, IterableOps, Obj, Parser,
    PrWriter, RData, RGlobal, Rest, Root, Str, Tab, Val,
};
...
fn len(arg: Val) -> GResult<Val> {
    let collection_len: usize = match arg {
        ...
        Val::Str(st) => st.len(),
        Val::Tab(tab) => tab.len(),
        Val::GIter(giter) => return giter.len().into_val(),
        arg => bail!(
            "argument is {} rather than an arr, str or tab",
            arg.a_type_name()
        ),
    };
...
fn clear(arg: Val) -> GResult<Val> {
    match arg {
        ...
        arg => bail!(
            "received {} rather than an arr, str or tab",
            arg.a_type_name()
        ),
    }
...
fn hasp(arg: Val, key: Val) -> GResult<bool> {
    match arg {
        ...
        Val::Obj(obj) => match key {
            Val::Sym(key_name) => Ok(obj.has(key_name)?),
            _ => Ok(false),
        },
        Val::Class(class) => match key {
...
fn remove(coll: Val, key: Val) -> GResult<Val> {
    match coll {
        ...
        val => bail!(
            "expected an arr, str or tab, but received {}",
            val.a_type_name()
        ),
    }
}
...
fn remove_opt(coll: Val, index: Val) -> GResult<Option<Val>> {
    match coll {
        ...
        Val::Tab(tab) => tab.remove_if_present(index),
...
fn access(coll: Val, index: Val) -> GResult<Val> {
    match coll {
        ...
        Val::Tab(tab) => tab.get(index),
        Val::Obj(obj) => match index {
...
fn set_access(coll: Val, index: Val, new_value: Val) -> GResult<()> {
    match coll {
        ...
        Val::Tab(tab) => tab.set(index, new_value),
        Val::Obj(obj) => match index {
...
fn access_opt(coll: Val, index: Val) -> GResult<Option<Val>> {
    match coll {
        ...
        Val::Tab(tab) => tab.get_if_present(index),
        Val::Obj(obj) => match index {
...
fn access_slice(deq: Deque, i0: Option<i32>, i1: Option<i32>) -> GResult<Deque> {
    ...
    let adj1 = if i1 < 0 { len + i1 } else { i1 };
        ...
        adj0 <= adj1 && adj0 >= 0 && adj0 <= len && adj1 >= 0 && adj1 <= len,
        "invalid slice indexes: start {}, end {}",
        i0,
        i1
    );
...
fn set_access_slice(deq: Deque, i0: Option<i32>, i1: Option<i32>, src: Iterable) -> GResult<()> {
...
fn sort_mut(deq: Deque, ord: Option<Callable>) -> GResult<()> {
    match ord {
        ...
        Some(Callable::RFn(ref rfn))
            if Root::ptr_eq(rfn, Std::borrow().ord_rfn.as_ref().unwrap()) =>
        {
            deq.sort()
        }
        Some(ord) => deq.sort_by(|v0, v1| match glsp::call(&ord, (v0, v1))? {
    ...
    match haystack {
        ...
        Deque::Str(st) => match needle {
            ...
            Val::Str(needle) => {
                    ...
                    needle.len() > 0,
                    "substring search requires a non-empty str"
                );
